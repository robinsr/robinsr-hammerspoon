...
```lua
fun(item: <T:any>, index: integer):string
```
...
```lua
(method) List:any(fn: fun(item: <T:any>, index: integer):boolean)
  -> boolean
```
...
```lua
(method) List:concat(newitems: any[])
  -> List
```
...
```lua
(method) List:every(fn: fun(item: <T:any>, index: integer):boolean)
  -> boolean
```
...
```lua
(method) List:filter(fn: PredicateFn<<T>>)
  -> The: List
```
...
```lua
function Collection.findById(col: any, id: string)
  -> any
```
...
```lua
(method) List:first(fn: fun(item: <T:any>, index: integer):boolean)
  -> The: <T:any>|nil
```
...
```lua
(method) List:forEach(fn: fun(item: <T:any>, index: integer):nil)
  -> List
```
...
```lua
(method) List:groupBy(fn: string|fun(item: <T:any>, index: integer):string, ...any)
  -> table
```
...
```lua
(method) List:join(separator?: string)
  -> string
```
...
```lua
(method) List:map(fn: MappingFn<any, <O:any>>)
  -> List
```
...
```lua
function Collection.new(c: any, items: any[])
  -> Collection
```

@*param* `items` — List of commands to search

## pack


```lua
function List.pack(...any)
  -> List
```
...
```lua
(method) List:reduce(init: <R:any>, reducerFn: fun(memo: <R:any>, item: <T:any>, index: integer):<R:any>)
  -> The: <R:any>
```
...
```lua
function Collection.where(col: any, tabl: table)
  -> any
```
...
```lua
function Table.contains(tabl: table, elem: any)
  -> boolean
```
...
```lua
function Table.extract(tabl: table, keys: string[])
  -> table
```
...
```lua
function Table.get(tabl: table, ...string)
  -> Either: any
```
...
```lua
function Table.has(tabl: table, key: string, strict?: boolean)
  -> boolean
```
...
```lua
function Table.haspath(tabl: table, path: string, nilMsg?: string)
  -> boolean
```
...
```lua
function Table.isEmpty(tabl: table)
  -> boolean
```
...
```lua
function Table.keys(tabl: table, strict?: boolean)
  -> List: string[]
```
...
```lua
function Table.pick(tabl: table, keys: string[])
  -> table
```
...
```lua
function Table.values(tabl: table, strict?: boolean)
  -> list: any[]
```
...
```lua
fun(item: <T:any>, index: integer):nil
```
...
```lua
(method) Optional:new(val: <T>|nil, msg?: string)
  -> Optional<<T>>
```
...
```lua
(method) Optional:orElse(fallback: <T>)
  -> <T>
```
...
```lua
(method) Optional:orElseGet(supplier: fun():<T>)
  -> <T>
```
...
```lua
function Paths.mod(modname: string)
  -> string
```
...
```lua
fun(item: <T:any>, index: integer):boolean
```
...
```lua
fun(memo: <R:any>, item: <T:any>, index: integer):<R:any>
```
...
```
 ...
 sets __newindex such that new properties cannot be set
